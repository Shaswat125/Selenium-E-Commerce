...
if __name__ == "__main__":
    try:
        ...
        # Going for 4 ratings and above
        SeleniumPlayer.click(source_xpath.get("rating4"))
        time.sleep(1)
        # Going for 256 GB Storage
        SeleniumPlayer.click(source_xpath.get("expand_filter").replace('Primary Camera', 'Internal Storage'))
        time.sleep(1)
        SeleniumPlayer.click(source_xpath.get("Internal_Storage"))
        time.sleep(1)
        # Going for 64 MP Camera
        SeleniumPlayer.click(source_xpath.get("expand_filter"))
        time.sleep(1)
        SeleniumPlayer.click(source_xpath.get("Primary_camera"))
        # Open a File to store these details

        with open("PhoneData.txt", "a", encoding='utf-8') as file:
